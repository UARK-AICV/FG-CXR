...
from tools.dataset.mimc_cxr_chen import TaskSubsetHeatmap
...
class GenXAIMIMICXRChen(LightningModule):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.map_direction = {
            ...
            "lower right": 5,
            "heart": 6,
        }  # match with the order of labels in extract_direction_feature.py
        """
        Evaluation metrics
        ...
        self.test_chexbert_metrics = CheXbertMetrics(
            bert_path="bert-base-uncased",
            checkpoint_path="stanford/chexbert/chexbert.pth",
            ckpt_dir=self.ckpt_zoo_dir,  # 'checkpoints',
            mbatch_size=self.mbatch_size,  # 32,
            exp_dir=self.exp_dir_trial,  # 'experiments_gaze/mimic_cxr/train_cxr_gaze_latent/trial_4',
        )

        # Report logging:
        self.val_report_logger = ReportLogger(
            exp_dir=self.exp_dir_trial, split="val_reports"
        )
        self.test_report_logger = ReportLogger(
            exp_dir=self.exp_dir_trial, split="test_reports"
        )
        ...
        # the new transformer package break this part in the code.
        class DummyEncoder:
        ...
        class Decoder(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.encoder_decoder = transformers.EncoderDecoderModel(
                    encoder=dummy_encoder, decoder=decoder
                )

        self.decoder = Decoder()

        # TODO: load pretrained here.
        if load_pretrained:
            pass
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            self.train_set = TaskSubsetHeatmap(
            ...
            self.val_set = TaskSubsetHeatmap(
        ...
        if stage == "test" or stage is None:
            self.test_set = TaskSubsetHeatmap(
                examples=self.format_examples(examples["test"]),
                # examples=self.format_examples(examples["train"]),
                tokenizer=self.tokenizer,
                decoder_max_len=self.decoder_max_len,
    ...
    def format_examples(self, examples):
        for i in examples:
            ...
            i["heatmap_file_path"] = i.pop("heatmap_path")
            i["label"] = i.pop("report")
            i["direction"] = self.map_direction[i["direction"]]
            i["image_file_path"] = [
                os.path.join(self.dataset_dir, j) for j in i["image_file_path"]
            ]
            i["heatmap_file_path"] = [
                os.path.join(self.heatmap_dataset_dir, j)
                for j in i["heatmap_file_path"]
            ]
    ...
    def configure_optimizers(self):
        ...
        optimiser = {
            "optimizer": torch.optim.AdamW(grouped_parameters, lr=self.decoder_lr)
        }
        return optimiser
    ...
    def get_direction_feature_512(self, left_right_heart_token, directions):
        ii = left_right_heart_token[directions.view(-1, 1)]
        return ii  # shape is [batch_size,1, 512]

    def forward(
        self, images, directions, heatmaps, decoder_input_ids, decoder_attention_mask
    ):
        """
        https://lightning.ai/docs/pytorch/stable/common/lightning_module.html#forward
        images shape is [batch_size, 3, 384, 384], the encoder feature shape is [batch_size, 384/16*384/16, 768] = [batch_size, 576, 768]
        """
        encoder_outputs = self.encoder_forward(images)
        heatmaps = einops.rearrange(heatmaps, "b h w -> b (h w) 1")
        encoder_outputs.last_hidden_state = encoder_outputs.last_hidden_state * heatmaps
        direction_feature = self.left_right_heart_token_projection(
            self.get_direction_feature_512(self.left_right_heart_token, directions)
        )
        # cat fusion
        encoder_outputs.last_hidden_state = torch.cat(
            [encoder_outputs.last_hidden_state, direction_feature], dim=1
        )  # shape is [batch_size, 576+1, 768]
    ...
    def generate(self, num_beams, images, directions, heatmaps):
        """
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Logging:
        self.log_dict(
            {"train_loss": loss}, on_step=True, on_epoch=True, batch_size=y_hat.shape[0]
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output_ids = self.generate(
            1, batch["encoder_images"], batch["direction"], batch["heatmap"]
        )
        ...
        self.val_coco_metrics.update(
            generated, [[i] for i in batch["labels"]], ids=batch["id"]
        )
    ...
    def on_validation_epoch_end(self):
        ...
        self.log_dict(
            {f"val_{k}": v for k, v in scores.items()}, on_step=False, on_epoch=True
        )
    ...
    def on_test_epoch_end(self):
        ...
        self.log_dict(
            {f"test_{k}": v for k, v in scores.items()}, on_step=False, on_epoch=True
        )
